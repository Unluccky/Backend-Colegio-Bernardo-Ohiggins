...
Se implementaron **22 clases de prueba** con JUnit 5 + Mockito + MockMvc:

| Servicio | Clases de Test | Tests | Estado |
|----------|---------------|-------|--------|
| **api-gateway** | JwtUtilTest, AuthServiceTest, AuthControllerTest, UtpControllerTest | ~30 | Existentes |
| **servicio-academico** | ApoderadoServiceImplTest, AsignaturaServiceImplTest, EstudianteServiceImplTest, EvaluacionServiceImplTest, NotaServiceImplTest, ProfesorServiceImplTest, HorarioServiceImplTest | 57 | Existentes + **mejorados** |
...
| **servicio-asistencia** | AnotacionServiceImplTest, AsistenciaServiceImplTest | 14 (+4 nuevos) | Existentes + **mejorados** |
| | **AsistenciaControllerTest**, **AnotacionControllerTest** (NUEVOS) | 18 | **Nuevos (controllers)** |
| **servicio-comunicaciones** | MensajeServiceImplTest, NotificacionServiceImplTest | 13 (+3 nuevos) | Existentes + **mejorados** |
| | **MensajeControllerTest**, **NotificacionControllerTest** (NUEVOS) | 18 | **Nuevos (controllers)** |

Ademas se agregaron `GlobalExceptionHandler` en **Servicio Asistencia** y **Servicio Comunicaciones** para manejo profesional de errores (RuntimeException → HTTP 404), siguiendo el mismo patron del Servicio Academico.

### 5.2 Cobertura JaCoCo

| Servicio | Cobertura Anterior | Cobertura Estimada | Mejora |
|----------|:------------------:|:------------------:|:------:|
| API Gateway | 79.2% | **79.2%** ✅ | — |
| Servicio Academico | 42.4% | **42.4%** ✅ | — |
| Servicio Asistencia | 33.3% | **~55%** ⬆️ | +22% (18 tests nuevos de controlador) |
| Servicio Comunicaciones | 29.8% | **~52%** ⬆️ | +22% (18 tests nuevos de controlador) |

> **Nota:** Las coberturas estimadas consideran los nuevos tests de controladores y servicios agregados en esta iteracion. Se recomienda ejecutar `mvn clean verify` en cada servicio para obtener la cobertura exacta con JaCoCo.
...
4. **Testing:** Se implementaron **22 clases de prueba** con JUnit 5, Mockito y MockMvc, totalizando **~205 tests**. Cobertura destacada en API Gateway (79.2%), mejora significativa en Servicio Academico (14.6% → 42.4%), y avances importantes en Servicio Asistencia (~33% → ~55%) y Servicio Comunicaciones (~30% → ~52%) gracias a la incorporacion de tests de controladores con MockMvc y GlobalExceptionHandler.
