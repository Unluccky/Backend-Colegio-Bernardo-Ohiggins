...
class AnotacionServiceImplTest {
    ...
    @Test
    @DisplayName("guardar - continúa si Feign falla por indisponibilidad del servicio (fallback retorna null)")
    void guardar_feignNoDisponible_continuaGuardando() {
        // Cuando el Circuit Breaker está activo y el servicio no responde,
        // el fallback (AcademicoFeignClientFallback) retorna null
        when(academicoClient.buscarEstudiantePorId(10L)).thenReturn(null);
        when(repo.save(any(Anotacion.class))).thenReturn(anotacionPositiva);

        Anotacion resultado = service.guardar(anotacionPositiva);

        assertThat(resultado).isNotNull();
        assertThat(resultado.getDescripcion()).isEqualTo("Excelente participación");
        verify(repo, times(1)).save(anotacionPositiva);
    }

    @Test
    @DisplayName("guardar NEGATIVA - Feign lanza NotFound al buscar apoderados pero igual publica evento con apoderadoId=null")
    void guardar_anotacionNegativa_apoderadosNotFound_publicaEventoSinApoderado() {
        when(academicoClient.buscarEstudiantePorId(anyLong())).thenReturn(null);
        when(repo.save(any(Anotacion.class))).thenReturn(anotacionNegativa);
        doThrow(FeignException.NotFound.class)
                .when(academicoClient).buscarApoderadosPorEstudiante(anyLong());
        ...
        assertThat(evento.getApoderadoId()).isNull();
        assertThat(evento.getEstudianteId()).isEqualTo(10L);
        assertThat(evento.getTipo()).isEqualTo("NEGATIVA");
        verify(academicoClient).buscarApoderadosPorEstudiante(10L);
    }
